...
if (isset($_POST['add_sales'])) {
    ...
    $sql = "INSERT INTO sales (product_name, quantity, price, total, sale_date, user_id) 
            VALUES (?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("siddsi", $product, $quantity, $price, $total, $date, $user_id);
    ...
    if ($stmt->execute()) {
        ...
        $log_sql = "INSERT INTO activity_log (user_id, action, table_name, record_id, changes) 
                    VALUES (?, 'add', 'sales', ?, ?)";
        $log_stmt = $conn->prepare($log_sql);
        $log_stmt->bind_param("iis", $user_id, $sale_id, json_encode($changes));
        $log_stmt->execute();
        $log_stmt->close();
        ...
        if ($update_stmt->execute()) {
            ...
            if (!empty($changes)) {
                $log_sql = "INSERT INTO activity_log (user_id, action, table_name, record_id, changes) 
                            VALUES (?, 'update', 'sales', ?, ?)";
                $log_stmt = $conn->prepare($log_sql);
                $log_stmt->bind_param("iis", $user_id, $id, json_encode($changes));
                $log_stmt->execute();
                $log_stmt->close();
            }
