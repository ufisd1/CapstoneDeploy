...
if (isset($_POST['add_egg_stock'])) {
    ...
    $sql = "INSERT INTO egg_inventory (admin_id, egg_type, size, quality, stock_quantity, production_date, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isssiss", $admin_id, $egg_type, $size, $quality, $stock_quantity, $production_date, $expiry_date);
    ...
    if ($stmt->execute()) {
        ...
        logActivity($conn, $admin_id, 'add', 'egg_inventory', $batch_id, $changes);
...
if (isset($_POST['update_egg_stock'])) {
    ...
    if ($old_result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            if (!empty($changes)) {
                include 'activity_log.php';
                logActivity($conn, $admin_id, 'update', 'egg_inventory', $batch_id, $changes);
            }
...
if (isset($_POST['delete_egg_stock'])) {
    ...
    if ($get_result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            include 'activity_log.php';
            logActivity($conn, $admin_id, 'delete', 'egg_inventory', $batch_id, $changes);
